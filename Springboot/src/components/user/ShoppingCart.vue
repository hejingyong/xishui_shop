<template>
  <div class="shopping-cart">
    ...
    <div class="product-box">
      <div class="nav-bar">
        <div class="container">
          <router-link to="/MallHome" style="text-decoration: none" title="溪水官网">
            <el-image class="logo" style="width: 55px;height: 55px;margin-top: 10px;margin-right: 15px;" src="http://127.0.0.1:9000/local/img/favicon.ico"></el-image>
          </router-link>
          ...
          this.$http.post('/order/cartOrder',this.$qs.stringify(send)).then((rep)=>{
            ...
            if(rep.data.code===200){
              ...
              this.$alert(msg, '下单成功', {
                ...
                type: 'success',
                callback: () => {
                  let orderName = this.getCurrentDate()+'溪水商城-'+this.baseInfo.userName+'购物车支付订单';
...
.shopping-cart .page-main .empty-cart-top{
  ...
  padding-left: 50%;
  margin: 65px 0 130px;
  background: url(http://127.0.0.1:9000/local/img/cart-empty.png) no-repeat 30% 0;
  color: #b0b0b0;
  overflow: hidden;
  text-align: left;
}
